  ...
  return (
    <div>
      ...
      <div className="header">
        <div>
          <h1>🔵 Dashboard UCAR Central</h1>
          <p>{dashboard?.total_institutions || 0} institutions · {dashboard?.active_alerts_count || 0} alertes actives · Top : {dashboard?.top_performer || 'N/A'}</p>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          {(dashboard?.critical_alerts_count || 0) > 0 && (
            <div className="badge badge-critical">🚨 {dashboard.critical_alerts_count} critique(s)</div>
          )}
          <div style={{ fontSize: 12, color: 'var(--muted)' }}>{new Date().toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</div>
        </div>
      </div>
